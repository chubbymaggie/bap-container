...
let round t x =
  try match t.enc ~signed:t.signed x |> t.dec with
    | Ok y -> assert_equal ~printer:t.show x y
...
let rounds seq t () = Sequence.iter seq  ~f:(round t)
...
let write ~expect value t () =
  ...
  assert_equal ~printer:ident buf expect

let size ~expect value t () =
  let leb = t.enc value in
  assert_equal ~printer:Int.to_string
...
let suite = "Leb128" >::: [
    ...
    "uint zero"      >:: (fun () -> round (int ~signed:false) 0);
    "int zero"       >:: (fun () -> round (int ~signed:true) 0);
    "uint32 zero"    >:: (fun () -> round (int32 ~signed:false) 0l);
    "int32 zero"     >:: (fun () -> round (int32 ~signed:true) 0l);
    "uint64 zero"    >:: (fun () -> round (int64 ~signed:false) 0L);
    "int64 zero"     >:: (fun () -> round (int64 ~signed:true) 0L);
