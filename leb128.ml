...
module Bits = EndianString.LittleEndian
...
let int =
  ...
  let read,write = match word_size with
    | W32 ->
      (fun buf -> Int32.to_int (get_int32 buf 0)),
      (fun buf x -> set_int32 buf 0 @@ safe Int32.of_int x)
    | W64 ->
      (fun buf -> Int64.to_int (get_int64 buf 0)),
      (fun buf x -> set_int64 buf 0 @@ Int64.of_int x) in
  {
